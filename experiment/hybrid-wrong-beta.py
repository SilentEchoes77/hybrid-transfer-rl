...
class HybridTransferAgent(AgentWithSimplePolicy):
        ...
        #phase 1
            ...
            for h in range(self.horizon-1,-1,-1):
                if h==self.horizon-1:
                    for s in range(self.S):
                        for a in range(self.A):
                            self.Whsa[h,s,a]=min(1,0.01*self.cb*4*self.horizon*self.bonus[s,a])
                else:
                    for s in range(self.S):
                        for a in range(self.A):
                            self.Whsa[h,s,a]=min(1,0.01*self.cb*4*self.horizon*self.bonus[s,a]+self.backW(s,a,h))
    
    def fit(self,budget,**kwargs):
        T=budget
        rewards=np.zeros(T//100)
        self.recognization_ended=False
        for t in range(T):
            # if ((t+1)%100)==0:
            #     print("episode", t+1)
            if ((t+1)%100)==0:
            ...
            if not self.recognization_ended:
                self.recognization_ended=self.stop_recognization()
            self.update_value()
            observation,info=self.env.reset()
            done=False
            step=0
    ...
    def stop_recognization(self):
        ...
        error_bound=3*np.sqrt(W_max)+W_max
        ended=False
        if error_bound<self.sigma*self.gap/8:
...
def hyrbid_beta(num):
    ...
    rewards=[]
    for i in range(5):
        agent=HybridTransferAgent(env=env,horizon=horizon,source_count=source_count,source_transition=source_P,cb=0.002)
        reward=agent.fit(budget=T)
        rewards.append(reward)
    rewards=np.array(rewards)
    mean=np.mean(rewards,axis=0)
    return [success,mean[-1]]


def main():
    agents=range(1,17) # fit N agent
    optimality=[]
    with multiprocessing.Pool(processes=16) as pool:
       optimality=pool.map(hyrbid_beta,agents)
    optimality_hybrid=np.array(optimality)
    np.save("../data/hybridopt",optimality_hybrid)
