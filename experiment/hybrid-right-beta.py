        ...
        #phase 1
            ...
            for h in range(self.horizon-1,-1,-1):
                if h==self.horizon-1:
                    for s in range(self.S):
                        for a in range(self.A):
                            self.Whsa[h,s,a]=min(1,0.005*self.cb*4*self.horizon*self.bonus[s,a])
                else:
                    for s in range(self.S):
                        for a in range(self.A):
                            self.Whsa[h,s,a]=min(1,0.005*self.cb*4*self.horizon*self.bonus[s,a]+self.backW(s,a,h))
...
# experiment
def fit_hybrid(num):
    print("fitting",num,"hybrid agent")
    env=NRoom(
        nrooms=1,
        remove_walls=False,
        room_size=4,
...
def main():
    ...
    with multiprocessing.Pool(processes=5) as pool:
       rewards=pool.map(fit_hybrid,agents)
    reward_hybrid=np.array(rewards)
    np.save("../data/hybridreward",reward_hybrid)
